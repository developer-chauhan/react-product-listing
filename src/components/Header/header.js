        ...
        <div className="w-full px-4 py-2 text-center">
          <form>
            <div className="flex items-center border-b border-b-2 border-grey py-2">
              <input className="appearance-none bg-transparent border-none w-full text-grey-darker mr-3 py-1 px-2 leading-tight focus:outline-none" type="text" placeholder="Search here" aria-label="search" onChange={(evt) => this.search(evt)} ref={input => input && input.focus()}/>
              <button className="flex-no-shrink border-transparent border-4 text-grey hover:text-grey-darker text-sm py-1 px-2 rounded" type="button" onClick={() => {this.props.setSearchText("");this.setState({search:false})}}>
                Cancel
              </button>
            </div>
          </form>
